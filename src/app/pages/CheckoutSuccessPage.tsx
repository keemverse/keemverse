  ...
  if (!state?.txRef) {
    return (
      <div className="min-h-screen flex items-center justify-center text-center px-6 bg-background">
        <div>
          <p className="font-serif text-2xl text-foreground mb-3">No order to show</p>
          <button
            onClick={() => navigate("/fashion/presets")}
            className="mt-4 rounded-full bg-foreground px-6 py-3 text-sm uppercase tracking-[0.18em] text-background hover:opacity-90 transition"
          >
            Back to Presets
          </button>
        </div>
    ...
    <div className="min-h-screen bg-background text-foreground">
      <main className="max-w-lg mx-auto px-5 md:px-8 pt-16 pb-24 text-center">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}>
          {status === "verifying" && (
            <>
              <p className="uppercase tracking-[0.35em] text-xs text-muted-foreground mb-4">One moment</p>
              <h1 className="font-serif text-3xl text-foreground mb-3">Confirming your payment…</h1>
              <p className="text-muted-foreground">This only takes a few seconds.</p>
            </>
          )}

          {status === "unlocked" && (
            <>
              <p className="uppercase tracking-[0.35em] text-xs text-muted-foreground mb-4">You're in</p>
              <h1 className="font-serif text-3xl text-foreground mb-3">Payment confirmed</h1>
          ...
          {status === "failed" && (
            <>
              <p className="uppercase tracking-[0.35em] text-xs text-muted-foreground mb-4">Hold on</p>
              <h1 className="font-serif text-3xl text-foreground mb-3">We couldn't confirm that yet</h1>
